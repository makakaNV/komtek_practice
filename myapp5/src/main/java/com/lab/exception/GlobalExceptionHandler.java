...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleTypeMismatchException(MethodArgumentTypeMismatchException ex) {
        String message = "Неверный формат параметра '" + ex.getName() + "'. Необходим тип " + ex.getRequiredType().getSimpleName();
    ...
    public ResponseEntity<ErrorResponse> handleInvalidEnumValue(InvalidEnumValueException ex) {
        ...
        String message = "Некорректное значение для поля " + ex.getFieldName() + ". Допустимые значения: " + validValues;
        ErrorResponse errorResponse = new ErrorResponse(message, HttpStatus.BAD_REQUEST.value());
        return new ResponseEntity<>(errorResponse, HttpStatus.BAD_REQUEST);
    }
